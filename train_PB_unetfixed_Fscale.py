...
from utils.PB_utilsfixed_Fscale import Dataset, PB_Unet
...
if __name__=='__main__':
    ...
    for i in range(args.epochs):
        for s in range(len(dataset)):
            ...
            loss, y_im, sig_a, alpha = model.pretrain(sample['zyx'], sample['Y_init'], sig_init, a_init, mask=sample['mask_bool'])
    ...
    for i in range(args.pde_epochs):
        ...
        sigma_a = d_ad.Constant(constants[0].detach().cpu().numpy(), name = "sigma")
        alpha = d_ad.Constant(constants[1].detach().cpu().numpy(), name = "alpha")
