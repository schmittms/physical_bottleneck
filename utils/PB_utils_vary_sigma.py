...
class Dataset(torch.utils.data.Dataset):
    ### This should be equivalent to the class in PB_utils.py with the sigma_x switch turned off.
    def __init__(self, 
                 root,
                 indices=[0],
                 Y_init_scale=1e-3,
        ...
        self.form_reduced_dataset(indices=indices, sigma_x=sigma_x)
    ...
    def form_reduced_dataset(self, indices, sigma_x=False):
        ...
        for idx in indices:
            ...
            sig_init = np.linalg.norm(F, axis=0)
            sig_init = sig_init*self.sig_init_scale/sig_init[~np.isnan(sig_init)].max() + 0.0001 # Careful. Terrible abuse of notation Y_init=const, Yinit=image
            ...
            sig_init_torch = torch.tensor(sig_init[None,:,:], device='cpu', dtype=torch.float)
            mask_bool_torch = torch.tensor(mask[None,:,:], device='cpu', dtype=torch.bool)

            Jhat_np, control_arr, pde_forward, Y_FctSpace, Jhat, J = self.init_dolfin(Y_init, sig_init, F, mesh, element_degree=1)
            self.red_dataset.append({'zyx': zyx_torch, 
                       'F': F_torch, 
                       'Y_init': Y_init_torch, 
                       'sig_init': sig_init_torch, 
    ...
    def init_dolfin(self, Y_init, sig_init, F_img, mesh, element_degree=1):
        """
        ...
        def sigma(u, alpha, sigma_a):
            return (sigma_a + alpha*ufl.div(u))*ufl.Identity(2) + epsilon(u)
        ...
        def forward(Y, sigma_a, alpha, mesh):
            ...
            a = ufl.inner(sigma(u, alpha, sigma_a), epsilon(v))*ufl.dx + ufl.dot(Y*u,v)*ufl.dx 

            LHS = ufl.lhs(a)
            RHS = ufl.rhs(a)
            
            u = d_ad.Function(V, name="Sol")
            d_ad.solve(LHS == RHS, u)
        ...
        sig_mesh = proc.scalar_img_to_mesh(sig_init, Y_FctSpace)
        F_mesh = proc.vector_img_to_mesh(F_img, F_FctSpace)

        u = forward(Y_mesh, sig_mesh, alpha, mesh)   
        ...
        controlsig = d_ad.Control(sig_mesh)
...
class PB_Unet(nn.Module):
    def __init__(self, 
        ...
        self.convNextBlock.append(nn.Conv2d(3*hidden_channels, 2, 5, padding=2)) # 3*hidden because it takes 2*hidden from the latent space
    ...
    def make_scalar_net(self):
        ...
        self.sig_alpha_net.append(nn.ReLU())
        self.sig_alpha_net.append(nn.Linear(32, 1))
        return
    ...
    def pretrain(self, x, Y_init, sigma_a_init, alpha_init, mask=None):
        ...
        y_im, _, sig_im, _, constants = self.forward(x, None)

        print("SHAPES Y, Sig, Yinit, Siginit, MASK", y_im.shape, sig_im.shape, Y_init.shape, sigma_a_init.shape, mask.shape)
        
        if mask is None:
            loss = nn.MSELoss()(y_im, Y_init) +  nn.MSELoss()(sig_im, sigma_a_init) + self.imag_regularization*imag_part.abs().sum()
        else:
            loss = nn.MSELoss()(y_im[mask!=0], Y_init[mask!=0]) + nn.MSELoss()(sig_im[mask!=0], sigma_a_init[mask!=0]) #+ imag_part.abs().sum()

        loss += (constants[0] - alpha_init)**2
        loss.backward()
        ...
        return loss, y_im.detach().cpu().numpy(), sig_im.detach().cpu().numpy(), constants[0].detach().cpu().numpy()
    
    def step(self, x, Jhat, len_control_arr, Y_FctSpace):
        self.batch_counter += 1
        if self.batch_counter == 1: self.opt.zero_grad()
        
        y_im, y_mesh_vals, sig_im, sig_mesh_vals, constants = self.forward(x, Y_FctSpace)
        
        #assert len(y_mesh_vals) + len(sig_mesh_vals) + len(constants.squeeze()) == len_control_arr
        
        control_arr = np.zeros(len_control_arr)
        
        control_arr[:len(y_mesh_vals)] = y_mesh_vals.detach().cpu().numpy()
        control_arr[len(y_mesh_vals):len(y_mesh_vals)+len(sig_mesh_vals)] = sig_mesh_vals.detach().cpu().numpy()
        control_arr[-1] = constants[0] # alpha

        J = Jhat(control_arr) # Tape gets two z
        dJdy = Jhat.derivative(control_arr, forget=True, project=False)

        y_mesh_vals.backward(gradient=torch.tensor(dJdy[:len(y_mesh_vals)], device=torch.device('cuda:0'))/self.batch_size, retain_graph=True) #+ self.y_regularization*y_mesh_vals.abs())
        sig_mesh_vals.backward(gradient=torch.tensor(dJdy[len(y_mesh_vals):len(y_mesh_vals)+len(sig_mesh_vals)], device=torch.device('cuda:0'))/self.batch_size) #+ self.y_regularization*y_mesh_vals.abs())
        constants[0].backward(gradient=torch.tensor(dJdy[-1], device=torch.device('cuda:0'))/self.batch_size, retain_graph=True)
        
        if self.batch_counter == self.batch_size:
            self.batch_counter = 0
            self.opt.step()
            self.sch.step()
        return constants, y_im, y_mesh_vals, sig_im, sig_mesh_vals, J, dJdy #, pvals.detach().abs().mean().numpy(), pgrad.detach().abs().mean().numpy()#, xgrad, yimgrad
    ...
    def forward(self, x, Y_FctSpace, const_grad=False):
        ...
        #print("x shape", x.shape) (1, 2, H, W)
        y_sig_im = self.blur(x[0].exp()) # should have shape [B, H, W]
        #print("Y sig im shape", y_sig_im.shape)
        y_im = y_sig_im[0].unsqueeze(0)#.squeeze()
        sig_im = y_sig_im[1].unsqueeze(0)
        #sig_im = self.blur(x[1].exp()) # should have shape [B, H, W]
        
        if Y_FctSpace is not None: 
            y_mesh_vals = proc.scalar_img_to_mesh(y_im.squeeze(), Y_FctSpace, vals_only=True)
            sig_mesh_vals = proc.scalar_img_to_mesh(sig_im.squeeze(), Y_FctSpace, vals_only=True)

            return y_im, y_mesh_vals, sig_im, sig_mesh_vals, constants
        else: 
            return y_im, None, sig_im, None, constants
    ...
    def plot_sig(self, epoch, zyx, Y):
        fig, ax = plt.subplots(1, 4, figsize=(2*4, 2*1), dpi=144)

        ax[0].imshow(zyx/zyx.max(), origin='lower', vmax=0.3, cmap='gray')
        ax[1].imshow(Y, origin='lower', vmax=None, vmin=0, cmap='Blues')
        ax[2].imshow(Y, origin='lower', vmax=1e-3, vmin=0, cmap='Blues')
        ax[3].imshow(Y, origin='lower', vmax=1e-5, vmin=0, cmap='Blues')
        ...
        fig.subplots_adjust(wspace=0.01, hspace=0.01)
        self.logger.add_figure("sig/model", fig, close=True, global_step=epoch)
        return
