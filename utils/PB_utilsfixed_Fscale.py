...
class Dataset(torch.utils.data.Dataset):
    ...
    def init_dolfin(self, Y_init, F_img, mesh, element_degree=1):
        ...
        F_mesh = proc.vector_img_to_mesh(F_img*(0.46*1e-9), F_FctSpace) # Now in units of N/px^2
...
class PB_Unet(nn.Module):
    ...
    def pretrain(self, x, Y_init, sigma_a_init, alpha_init, mask=None):
        ...
        loss += (torch.log(constants[0]) - np.log(sigma_a_init))**2 + (constants[1] - alpha_init)**2
        loss.backward()
    ...
    def plot_F(self, epoch, F_mesh, Y_mesh, u, mesh):
        ...
        maxFexp = np.max([np.linalg.norm(u) for u in U])
        ...
        maxFpred = np.max([np.linalg.norm(u) for u in U])
