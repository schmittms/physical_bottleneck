...
class Dataset(torch.utils.data.Dataset):
    ...
    def form_reduced_dataset(self, indices):
        ...
        for idx in indices:
            ...
            mc = mesh.coordinates()
            mc[:, :] *= 0.68*1e-6 # all meshes are now in [m]
            ...
            Y_init = Y_init*self.Y_init_scale/Y_init[~np.isnan(Y_init)].max() + self.Y_init_scale*0.001 # Careful. Terrible abuse of notation Y_init=const, Yinit=image
    ...
    def init_dolfin(self, Y_init, F_img, mesh, element_degree=1):
        ...
        def sigma(u, E_cell, poisson):
            return (E_cell/(1+poisson))*((poisson/(1-2*poisson))*ufl.div(u)*ufl.Identity(2) + epsilon(u))

        el = ufl.VectorElement("CG", mesh.ufl_cell(), element_degree)
        V = dlf.FunctionSpace(mesh, el)

        def forward(Y, sigma_a, E_cell, poisson, mesh):
            ...
            a = ufl.inner(h_cell*sigma(u, E_cell, poisson), epsilon(v))*ufl.dx + ufl.dot(Y*u,v)*ufl.dx 
            L = ufl.dot(-h_cell*sigma_a*n, v)*ufl.ds 
        ...
        controlE = d_ad.Control(E_cell)
        controlnu = d_ad.Control(poisson)

        Jhat_np = pyad.ReducedFunctionalNumPy(J, [controlY, controlsig, controlE, controlnu])
        Jhat = pyad.ReducedFunctional(J, [controlY, controlsig, controlE, controlnu])
...
class PB_Unet(nn.Module):
    def __init__(self, 
        ...
        self.blur = transforms.GaussianBlur(blur_radius, sigma=(2., 4.))
    ...
    def make_scalar_net(self):
        ...
        self.sig_alpha_net.append(nn.ReLU())
        self.sig_alpha_net.append(nn.Linear(32, 3))
        return
        
        
    def pretrain(self, x, Y_init, sigma_a_init, E_cell_init, poisson_init, mask=None):
        self.batch_counter += 1
        if self.batch_counter == 1: self.opt.zero_grad()
        
        y_im, _, constants = self.forward(x, None)

        nan_mask = (torch.isfinite(torch.log(y_im)))*(torch.isfinite(torch.log(Y_init)))
        
        if mask is None:
            loss = nn.MSELoss()(y_im, Y_init) + self.imag_regularization*imag_part.abs().sum()
        else:
            loss = nn.MSELoss()(torch.log(y_im[(mask!=0)*nan_mask]), torch.log(Y_init[(mask!=0)*nan_mask])) #+ imag_part.abs().sum()

        loss += (constants[0] - np.log(sigma_a_init))**2 + (constants[1] - np.log(E_cell_init))**2 + ((0.75*torch.tanh(constants[2])-0.25) - poisson_init)**2
        loss.backward()
        ...
        return loss, y_im.detach().cpu().numpy(), constants[0].detach().cpu().numpy(), constants[1].detach().cpu().numpy(), constants[2].detach().cpu().numpy()
    ...
    def step(self, x, Jhat, len_control_arr, Y_FctSpace):
        ...
        control_arr[-3] = constants[0].exp() # sig
        control_arr[-2] = constants[1].exp() # E_cell
        control_arr[-1] = 0.75*torch.tanh(constants[2]) - 0.25 # poisson ratio, in range [-1, 1/2]

        J = Jhat(control_arr) # Tape gets two z
        dJdy = Jhat.derivative(control_arr, forget=True, project=False)

        y_mesh_vals.backward(gradient=torch.tensor(dJdy[:-3], device=torch.device('cuda:0'))/self.batch_size) #+ self.y_regularization*y_mesh_vals.abs())
        constants[0].backward(gradient=torch.tensor(dJdy[-3], device=torch.device('cuda:0'))/self.batch_size, retain_graph=True)
        constants[1].backward(gradient=torch.tensor(dJdy[-2], device=torch.device('cuda:0'))/self.batch_size, retain_graph=True)        
        constants[2].backward(gradient=torch.tensor(dJdy[-1], device=torch.device('cuda:0'))/self.batch_size)        
    ...
    def plot_F(self, epoch, F_mesh, Y_mesh, u, mesh):

        fig, ax = plt.subplots(1, 2, figsize=(2*2, 2*1), dpi=144)

        im_ex = ax[0].tripcolor(*self.tricolor(ufl.sqrt(ufl.dot(F_mesh,F_mesh)), mesh), vmin=0,vmax=None, cmap='inferno')
        im_pr = ax[1].tripcolor(*self.tricolor(ufl.sqrt(ufl.dot(u*Y_mesh,u*Y_mesh)), mesh), vmin=0,vmax=None, cmap='inferno')

        arrow_width = 0.005
        dns = 2
        thresh=0.0
        X,U=self.triquiver( F_mesh, mesh)
        X = np.asarray([x[::dns] for x in X]).T
        U = np.asarray([x[::dns] for x in U]).T
        #print(" U SHAPE ", U.shape) # (1041, 2)
        ...
        #print(U.shape) # (2, 1041)
        ax[0].quiver(*X, *U, color='w', width=arrow_width, scale=None, alpha=0.8)

        maxFexp = np.max([np.linalg.norm(u) for u in U.T])


        X,U=self.triquiver( Y_mesh*u, mesh)
        X = np.asarray([x[::dns] for x in X]).T
        U = np.asarray([x[::dns] for x in U]).T
        X = np.asarray([x for u,x in zip(U,X) if np.linalg.norm(u, axis=0)>thresh]).T
        U = np.asarray([u for u in U if np.linalg.norm(u, axis=0)>thresh]).T
        ax[1].quiver(*X, *U, color='w', width=arrow_width, scale=None, alpha=0.8)

        maxFpred = np.max([np.linalg.norm(u) for u in U.T])

        for a in ax.flat: a.axis('off')

        ax[0].set_title(f'F exp\nmax: {maxFexp:0.1e}')
        ax[1].set_title(f'F pred\nmax: {maxFpred:0.1e}')

        fig.subplots_adjust(wspace=0.01, hspace=0.01)
        self.logger.add_figure("F/model", fig, close=True, global_step=epoch)
        return
